  ...
  class Contracts extends Request {
    public function execute () {
      ...
      $result = $this->db->execute_query ("
        ...
        FROM proposals p INNER JOIN users u ON u.id = p.id_student WHERE p.id_professor = ? ORDER BY p.create_at DESC
      ", [$id_user]);

      $proposals = array ();
      while ($row = $result->fetch_assoc()) {

        $student = $this->db->execute_query ("SELECT id, email, full_name, image_url FROM users WHERE id = ? AND activate = 1", [$row["id_student"]]);
      ...
      $this->send_message('Contracts load successfully!', 200, $body);
    }
  }

  $job = new Contracts('GET', true);
	$job->request();
?>
